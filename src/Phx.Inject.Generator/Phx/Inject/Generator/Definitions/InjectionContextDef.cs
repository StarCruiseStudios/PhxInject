    ...
    internal record InjectionContextDef(
        ...
        public class Builder : IBuilder {
            ...
            public InjectionContextDef Build(
                ...
                var specDescs = context.Specifications.Values.ToImmutableList();
                ...
                var specContainerDefs = specDescs.Select(
                        specDesc => specContainerDefBuilder.Build(specDesc, generationContext))
                    .ToImmutableList();

                var dependencyImplementationDefs = injectorDesc.ChildFactories
                    .Select(
                    ...
                    .Select(
                        dependencyType => generationContext.GetDependency(
                            dependencyType,
                            injectorDesc.Location))
                    .Select(
                        dependency => dependencyImplementationDefBuilder.Build(
                            dependency,
                            generationContext))
                    .ToImmutableList();
